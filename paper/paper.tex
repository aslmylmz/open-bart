...
capacity---so high that almost no participant reaches it, collapsing the task into
a one-dimensional index of raw risk exposure. \emph{Dynamic Hazard Rate BART}
replaces the classic uniform burst model with a sequence of independent Bernoulli
...
reckless play. The software bundles a React/Next.js game client, a Python scoring
...
That target is so high that essentially no participant reaches it; because nearly
everyone stops well short of the optimum, the number of pumps and the money
earned become almost collinear, and the task degrades into a one-dimensional
proxy for raw risk exposure rather than \emph{calibrated} risk-taking
\citep{frey2017}.
...
approximately $\sqrt{N}$---an attainable value---so the task can separate
over-conservative, optimal, and over-exposed participants. The repository contains
...
calibration---it can only index how far a participant pushed. Simulation work has
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  the task to distinguish over-conservative, optimal, and over-exposed
...
For a balloon of capacity $N$, the probability of surviving $s$ successive pumps
is the product of the per-pump survival probabilities,
%
\begin{equation}
