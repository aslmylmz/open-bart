...
- The Master CSV data dictionary matches the real columns; action controls stay
  measurement-neutral.
- Kiosk lock + practice banner are observed in a real Tauri run. (Issue 64/F15 fixed
  the ungranted window setters; issue 60 then verified fullscreen engagement in a real
  `tauri dev` build via screenshot — see `VERIFY-WINDOWS.md` § Recorded observation and
  the `app/e2e/verify-kiosk-macos.sh` helper.)
- Every v1.0.0 `study.json` still validates; the four gates stay green throughout.
