...
Models:
    ...
    EngineStamp     — Server-authored per-session engine provenance
    SessionEnvelope — Per-session identity + provenance file (session.json)
...
class EngineStamp(BaseModel):
    """Per-session engine provenance (DATA-SPEC §3.1) — a lossless per-session
    copy of the app/engine/platform triple the study-level provenance record
    assembles. Server-authored: ``write_output`` stamps what actually ran at
    write time, ignoring anything a client claims. Per-session because the
    study-level record is last-writer-wins across stations and a write-time
    fact can never be backfilled."""

    app_version: str = Field(
        ...,
        description=(
            "The instrument (app) version that recorded this session. "
            "Currently identical to engine_version (released in lockstep); "
            "recorded separately so a future app/engine split stays "
            "representable."
        ),
    )
    engine_version: str = Field(
        ...,
        description=(
            "The scoring-engine version that ran this session — drives the "
            "Hub's verify-grading of stored metrics against re-scored ones."
        ),
    )
    platform: str = Field(
        ...,
        description=(
            "The OS/platform string of the machine that ran this session — "
            "per-station attribution that the last-writer-wins study-level "
            "provenance record cannot preserve."
        ),
    )


class SessionEnvelope(BaseModel):
    """The per-session ``*_session.json`` file (DATA-SPEC §3, Decision A):
    identity, design assignment, and per-session provenance in one
    purpose-built model, kept separate from the gameplay ``GameSession`` (the
    raw telemetry stays in ``*_events.jsonl``). Every rebuild path reads this
    one model; fields added since v1.0.0 are optional, so older files load
    with them ``None`` — no schema_version bump."""
    ...
    station_id: Optional[str] = Field(
        default=None,
        description=(
            "the label of the station (machine) that recorded this session "
            "(DATA-SPEC §2.3); None on single-station data, where no station "
    ...
    id_source: Optional[Literal["generated", "manual"]] = Field(
        default=None,
        description=(
            "whether candidate_id was auto-generated by the ID screen's "
            "Generate button or hand-typed (DATA-SPEC §3.2); None on data "
            "recorded before — or without — the auto_participant_id option"
        ),
    )
    engine: Optional[EngineStamp] = Field(
        default=None,
        description=(
            "server-authored engine provenance (DATA-SPEC §3.1), stamped at "
            "write time; None on data recorded before the stamp existed — "
            "such sessions are ungraded at the Hub, never rejected"
        ),
    )
